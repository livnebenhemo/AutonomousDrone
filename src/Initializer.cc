...
namespace ORB_SLAM2 {
    ...
    double
    Initializer::CheckHomography(const cv::Mat &H21, const cv::Mat &H12, vector<bool> &vbMatchesInliers, double sigma) {
        const int N = mvMatches12.size();
    ...
    int Initializer::CheckRT(const cv::Mat &R, const cv::Mat &t, const vector<cv::KeyPoint> &vKeys1,
                             ...
                             const cv::Mat &K, vector<cv::Point3f> &vP3D, double th2, vector<bool> &vbGood,
                             double &parallax) {
        ...
        for (size_t i = 0, iend = vMatches12.size(); i < iend; i++) {
            ...
            double im1x, im1y;
            double invZ1 = 1.0 / p3dC1.at<float>(2);
            ...
            double im2x, im2y;
            double invZ2 = 1.0 / p3dC2.at<float>(2);
