...
namespace ORB_SLAM2 {
    ...
    int ORBmatcher::SearchByProjection(Frame &F, const std::vector<std::shared_ptr<MapPoint>> &vpMapPoints,
    ...
    int ORBmatcher::SearchByBoW(KeyFrame *pKF, Frame &F,
        ...
        auto vpMapPointsKF = pKF->mvpMapPoints;
        for (int i = 0; i < F.N; ++i) {
            vpMapPointMatches[i] = static_cast<std::shared_ptr<MapPoint>>(nullptr);
        }
        ...
        if (mbCheckOrientation) {
            ...
            for (int i = 0; i < HISTO_LENGTH; i++) {
                if (i == ind1 || i == ind2 || i == ind3)
                    continue;
                for (int j: rotHist[i]) {
                    vpMapPointMatches[j] = static_cast<std::shared_ptr<MapPoint>>(nullptr);
                    nmatches--;
                }
            }
        }
    ...
    int ORBmatcher::SearchByProjection(KeyFrame *pKF, const cv::Mat &Scw,
        ...
        std::unordered_map<std::shared_ptr<MapPoint>, unsigned long> spAlreadyFound;
    ...
    int ORBmatcher::SearchByBoW(KeyFrame *pKF1, KeyFrame *pKF2, std::vector<std::shared_ptr<MapPoint>> &vpMatches12) {
        const std::vector<cv::KeyPoint> &vKeysUn1 = pKF1->mvKeysUn;
        const DBoW2::FeatureVector &vFeatVec1 = pKF1->mFeatVec;
        std::unordered_map<size_t, std::shared_ptr<MapPoint>> vpMapPoints1 = pKF1->mvpMapPoints;
        const cv::Mat &Descriptors1 = pKF1->mDescriptors;

        const std::vector<cv::KeyPoint> &vKeysUn2 = pKF2->mvKeysUn;
        const DBoW2::FeatureVector &vFeatVec2 = pKF2->mFeatVec;
        std::unordered_map<size_t, std::shared_ptr<MapPoint>> vpMapPoints2 = pKF2->mvpMapPoints;
        const cv::Mat &Descriptors2 = pKF2->mDescriptors;

        vpMatches12 = std::vector<std::shared_ptr<MapPoint>>(vpMapPoints1.size(),
        ...
        if (mbCheckOrientation) {
            ...
            for (int i = 0; i < HISTO_LENGTH; i++) {
                if (i == ind1 || i == ind2 || i == ind3)
                    continue;
                for (int j: rotHist[i]) {
                    vpMatches12[j] = static_cast<std::shared_ptr<MapPoint>>(nullptr);
                    nmatches--;
                }
            }
        }
    ...
    int ORBmatcher::Fuse(KeyFrame *pKF, std::unordered_map<size_t, std::shared_ptr<MapPoint>> &vpMapPoints,
                         const float th) {
                ...
                if (pMPinKF) {
                    if (!pMPinKF->isBad()) {
                        if (pMPinKF->Observations() > pMP->Observations()) {
                            pMP->Replace(pMPinKF);
                            if (pMP->isBad()) {
                                pKF->mpMap->EraseMapPoint(pMP);
                            }
                        } else {
                            pMPinKF->Replace(pMP);
                            if (pMPinKF->isBad()) {
                                pKF->mpMap->EraseMapPoint(pMPinKF);
                            }
                        }
                    }
                } else {
    ...
    int ORBmatcher::Fuse(KeyFrame *pKF, const cv::Mat &Scw, const std::vector<std::shared_ptr<MapPoint>> &vpPoints,
                         float th,
        ...
        for (int iMP = 0; iMP < nPoints; iMP++) {
            const std::shared_ptr<MapPoint>& pMP = vpPoints[iMP];
    ...
    int ORBmatcher::SearchBySim3(KeyFrame *pKF1, KeyFrame *pKF2, std::vector<std::shared_ptr<MapPoint>> &vpMatches12,
    ...
    int ORBmatcher::SearchByProjection(Frame &CurrentFrame, Frame &LastFrame, const float th) const {
        ...
        for (int i = 0; i < LastFrame.N; i++) {
            ...
            if (pMP) {
                if (!LastFrame.mvbOutlier[i]) {
                    ...
                    float u = ORB_SLAM2::Frame::fx * xc * invzc + ORB_SLAM2::Frame::cx;
                    float v = ORB_SLAM2::Frame::fy * yc * invzc + ORB_SLAM2::Frame::cy;
        ...
        if (mbCheckOrientation) {
            ...
            for (int i = 0; i < HISTO_LENGTH; i++) {
                if (i != ind1 && i != ind2 && i != ind3) {
                    for (int j: rotHist[i]) {
                        CurrentFrame.mvpMapPoints[j] = static_cast<std::shared_ptr<MapPoint>>(nullptr);
                        nmatches--;
                    }
                }
            }
        ...
        for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
            ...
            if (pMP) {
                if (!pMP->isBad() && !sAlreadyFound.count(pMP)) {
                    ...
                    const float u = ORB_SLAM2::Frame::fx * xc * invzc + ORB_SLAM2::Frame::cx;
                    const float v = ORB_SLAM2::Frame::fy * yc * invzc + ORB_SLAM2::Frame::cy;
