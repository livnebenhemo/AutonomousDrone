...
namespace ORB_SLAM2 {
    ...
    void Map::save(Archive &ar, const unsigned int version) const {
        ...
        std::for_each(mspMapPoints.begin(), mspMapPoints.end(), [&ar](const std::shared_ptr<MapPoint> &pMapPoint) {
            if (!pMapPoint->isBad()) {
                ar & *pMapPoint;
            }
        });
        ...
        std::for_each(mvpKeyFrameOrigins.begin(), mvpKeyFrameOrigins.end(), [&ar](KeyFrame *pKeyFrameOrigin) {
            if (!pKeyFrameOrigin->isBad()) {
                ar & *pKeyFrameOrigin;
            }
        });
        // Pertaining to map drawing
        //nItems = mvpReferenceMapPoints.size();
        // std::cout << "$${INFO}mvpReferenceMapPoints size = %d " << nItems << std::endl;
        //ar & nItems;
...
// Explicit template instantiation
    ...
    void Map::InformNewBigChange() {
        //std:: std::unique_lock<std::mutex> lock(mMutexMap);
        mnBigChangeIdx++;
    }
        ...
        // if(false)
        {
            ...
            for (auto &mp: mapPoints) {
                ...
                auto pnt3d = mp->GetWorldPos().clone();
                {
                    ...
                    aligned_pos << align_pos.at<float>(0) << " " << align_pos.at<float>(1) << " "
                                << align_pos.at<float>(2) << std::endl;
                }
            }
