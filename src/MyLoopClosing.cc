...
namespace ORB_SLAM2 {
    ...
    bool LoopClosing::DetectLoop() {
        ...
        //change: at first it was 10
        //If the map contains less than 10 KF or less than 10 KF have passed from last loop detection
        if (mpCurrentKF->mnId < mLastLoopKFid + 1) {
            mpKeyFrameDB->add(mpCurrentKF);
            mpCurrentKF->SetErase();
            return false;
        }
        //ToDo: read about bud of words
        ...
        for (auto pCandidateKF : vpCandidateKFs) {
            ...
            for (int iG = 0, iendG = mvConsistentGroups.size(); iG < iendG; iG++) {
                set<KeyFrame *> sPreviousGroup = mvConsistentGroups[iG].first;

                bool bConsistent = false;
                for (auto kf : spCandidateGroup) {
    ...
    void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap) {
        ...
        for (auto pKF : CorrectedPosesMap) {
            ...
            matcher.Fuse(pKF.first, cvScw, mvpLoopMapPoints, 4, vpReplacePoints);
            unique_lock<mutex> lock(mpMap->mMutexMapUpdate);
