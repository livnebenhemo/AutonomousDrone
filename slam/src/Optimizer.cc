...
namespace ORB_SLAM2 {
    ...
    void Optimizer::BundleAdjustment(const std::vector<KeyFrame *> &vpKFs,
        ...
        for (auto pKF: vpKFs) {
            ...
            maxKFid = pKF->mnId > maxKFid ? pKF->mnId : maxKFid;
        }
    ...
    void Optimizer::filterBySphericalCoordinates(KeyFrame *pKF) {
        ...
        auto points = pKF->mvpMapPoints;
        auto amountOfPoints = points.size();
        for (auto &[i, mapPoint]: pKF->mvpMapPoints) {
            if (mapPoint && !mapPoint->isBad()) {
                ...
                if (pointsAndAngles.count(polarAngle)) {
                    if (pointsAndAngles.at(polarAngle).count(azimuthalAngle)) {
                        mapPoint->SetBadFlag();
                        pKF->mpMap->EraseMapPoint(mapPoint);
                        amountOfPoints--;
                    } else {
    ...
    void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, Map *pMap) {
        ...
        for (auto vpMPs: lLocalKeyFrames) {
            for (auto &[i, pMP]: vpMPs->mvpMapPoints) {
                if (pMP && !pMP->isBad())
        ...
        if (!vToErase.empty()) {
            for (auto erase: vToErase) {
                ...
                if (erase.second) {
                    ...
                    if (erase.second->isBad()) {
                        pMap->EraseMapPoint(erase.second);
                    }
                }
            }
        }
        ...
        for (auto localKeyFrame: lLocalKeyFrames) {
            auto vSE3 = dynamic_cast<g2o::VertexSE3Expmap *>(optimizer.vertex(localKeyFrame->mnId));
            localKeyFrame->SetPose(Converter::toCvMat(vSE3->estimate()));
        }
        //Points
        for (auto pMP: lLocalMapPoints) {
            auto vPoint = dynamic_cast<g2o::VertexSBAPointXYZ *>(optimizer.vertex(
    ...
    int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, std::vector<std::shared_ptr<MapPoint>> &vpMatches1,
        ...
        for (size_t i = 0; i < vpEdges12.size(); i++) {
            ...
            if (e12->chi2() > th2 || e21->chi2() > th2) {
                ...
                vpMatches1[idx] = static_cast<std::shared_ptr<MapPoint>>(nullptr);
