...
namespace ORB_SLAM2 {
    ...
    void KeyFrameDatabase::set_vocab(std::shared_ptr<ORBVocabulary> voc) {
        mpVoc = voc;
    }
...
// Explicit template instantiation
    ...
    void KeyFrameDatabase::add(KeyFrame *pKF) {
        ...
        // std::std::cout << "Adding keyframes to database." << std::std::endl;
    ...
    void KeyFrameDatabase::erase(KeyFrame *pKF) {
        ...
        for (DBoW2::BowVector::const_iterator vit = pKF->mBowVec.begin(), vend = pKF->mBowVec.end();
             vit != vend; vit++) {
            //std::list of keyframes that share the word
    ...
    std::vector<KeyFrame *> KeyFrameDatabase::DetectLoopCandidates(KeyFrame *pKF, float minScore) {
        ...
        for (std::list<std::pair<float, KeyFrame *> >::iterator it = lScoreAndMatch.begin(), itend = lScoreAndMatch.end();
             it != itend;
             it++) {
        ...
        for (std::list<std::pair<float, KeyFrame *> >
    ...
    std::vector<KeyFrame *> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F) {
        std::vector<KeyFrame *> lKFsSharingWords;
