...
//
// Created b rbdstudent on 15/06/2021.
//
...
std::vector<double> lastRelativeChangeStack{};
std::string fixComment = "down 40";
bool first = false;  // TODO : if I want True, need to add variable (BFS navigation & CSV)
...
AutonomousDrone::AutonomousDrone(std::shared_ptr<ctello::Tello> drone,
    ...
    this->pointToIterations = std::vector<std::pair<Point, int>>();
...
void AutonomousDrone::updateIteration(const Point &destination) {
    ...
    std::cout << "insert mapping between point to iteration : " << this->iteration << std::endl;
    this->pointToIterations.emplace_back(std::make_pair(destination, this->iteration));
...
void AutonomousDrone::rotateDrone(int angle, bool clockwise, bool buildMap, bool isHome) {
    if (!lowBattery && angle >= 3) {
        ...
        else if ((!localized  && !stop) || (!localized && metzuded)) {
        ...
        if (lossLocalization) {  // TODO : delete after check
            ...
            while (amountOfLostLocalizations) {
                rotateDrone(maxRotationAngle, clockwise, false, isHome);
                amountOfLostLocalizations--;
            }
            rotateDrone(angle, clockwise, true, isHome);
        }
    }
    else
...
void AutonomousDrone::howToRotate(int angle, bool clockwise, bool buildMap, bool isHome) {
    if (angle < 360) {
        if (angle < 30) {
            ...
            int modAngle = int(angle % maxRotationAngle);
            rotateDrone(modAngle, clockwise, buildMap, isHome);
            if (modAngle < 3 && isHome)
            {
                if (!clockwise) // TODO : check if work
                    relativeChange -= modAngle;
                else
                    relativeChange += modAngle;
            }
        }
...
void call_to_python_program(std::string data_path, int coreset_size, std::string output_path, std::string python_file_path){
...
void AutonomousDrone::getNavigationPoints(bool isExit) {
    ...
    auto exitPoints = getNavigationPoints_helper("/tmp/pointData0.csv"); // TODO : fix it!
    ...
    std::vector<Point> filteredExitPoints = std::vector<Point>{};
    for (const auto &point: exitPoints) {
        ...
        if (!toFilter)
            filteredExitPoints.push_back(point);
    }
    std::cout << "amount of filtered navigation points before star:" << filteredExitPoints.size() << std::endl;
    std::cout << "find optimal path" << std::endl;
    Navigation navigation;
    auto optPath = navigation.findOptPath(filteredExitPoints, currentLocation);
    if (optPath.size() > 0) // TODO : check if related to the extreme case
        exitPoints = optPath;
    else
...
void AutonomousDrone::getNavigationPointsBFS(bool isExit, const Point &base, const Point &home, const std::vector<Point> &cloud, int angle, double ratio) {
    std::cout << "getting navigation points" << std::endl;
    exitStayInTheAirLoop = false;
    currentRoom.points = std::vector<Point>{};
    ...
    auto dataset_path = extractCloudPathFromPoint(base);
    auto exitPoints = getNavigationPoints_helper(dataset_path);
    ...
    std::cout << "find optimal path" << std::endl;
    Navigation navigation;
    exitPoints = navigation.findOptPath(filteredExitPoints, currentLocation);
    if (!isManual) {
        std::cout << "amount of navigation points before star:" << exitPoints.size() << std::endl;
        exitPoints = navigation.createStarNavigation(exitPoints, currentLocation);
    }
...
void AutonomousDrone::monitorDroneProgress(const Point &destination, bool toHome) {
    ...
    closeThreshold = Auxiliary::calculateDistanceXY(currentLocation, destination) / 2.5; // TODO : change 2.5
    if (toHome)
        closeThreshold /= 2;
...
bool AutonomousDrone::navigateDrone(const Point &destination, bool rotateToFrameAngle, bool isHome) {
    ...
    if (Auxiliary::calculateDistanceXY(destination, currentLocation) < 0.4) {  // TODO : not hard-coded?
        ...
        if (isHome)  // clear the stack
        {
            ...
            relativeChange = lastRelativeChangeStack.back();
            lastRelativeChangeStack.pop_back();
        }
        else
            lastRelativeChangeStack.push_back(relativeChange);
        return true;
    }
            ...
            if (lowBattery) {
                ...
                if (!droneRotate && !isBlocked) {
                    if (!localized) {
                        ...
                        bool clockwise = howToRotateToFrame.second;
                        fixExtremeCase(clockwise); // TODO : need to check ut, think if I want
                    } else {
...
void AutonomousDrone::seeTheWorldWhenReachedPoint(int angle){
    std::cout << "starting explore" << std::endl;
    this->metzuded = true;
    int rotationAngle = 20;
    for (int i = 0; i < std::ceil(angle / rotationAngle); i++) {
        if (lowBattery) {
            switchBattery();
            lowBattery = false;
        }
        howToRotate(rotationAngle, true, true, false); // isHome=true because we don't want add commands to stack
        std::cout << "[right with triangulation] : we did: " << (i + 1) * rotationAngle  << " clockwise"<< std::endl;
    }
    for (int i = 0; i < std::ceil(angle / rotationAngle)-1; i++) {
        if (lowBattery) {
            switchBattery();
            lowBattery = false;
        }
        if (!localized  && i==0){
            std::cout << "[left without triangulation] : loss localization " << std::endl;
            manageAngleDroneCommand(rotationAngle, false, 10, 2);
            this->navigateDroneHomePath.push("cw "  + std::to_string(rotationAngle));
        }
        else
            howToRotate(rotationAngle, false, false, false); // isHome=true because we don't want add commands to stack
        std::cout << "[left without triangulation] : we did: " << (i + 1) * rotationAngle << " counter clockwise" << std::endl;
    }
    for (int i = 0; i < std::ceil(angle / rotationAngle); i++) {
        if (lowBattery) {
            switchBattery();
        ...
        howToRotate(rotationAngle, false, true, false); // isHome=true because we don't want add commands to stack
        std::cout << "[left with triangulation] : we did: " << (i + 1) * rotationAngle << " counter clockwise" << std::endl;
    }
    for (int i = 0; i < std::ceil(angle / rotationAngle)-1; i++) {
        if (lowBattery) {
            switchBattery();
        ...
        if (!localized && i==0){
            std::cout << "[right without triangulation] : loss localization " << std::endl;
            manageAngleDroneCommand(rotationAngle, true, 10, 2);
            this->navigateDroneHomePath.push("ccw "  + std::to_string(rotationAngle));
        }
        else
            howToRotate(rotationAngle, true, false, false); // isHome=true because we don't want add commands to stack
        std::cout << "[right without triangulation] : we did: " << (i + 1) * rotationAngle << "clockwise" << std::endl;
    }
    this->metzuded = false;
    // TODO : need global bundle adjustment ?
    if (!loadMap)
        orbSlamPointer->GetLoopClosing()->RunGlobalBundleAdjustment(orbSlamPointer->GetMap()->GetMaxKFid());
}
...
std::vector<Point> AutonomousDrone::extractCloudFromPoint(const Point &base){
    int csv_index = -1;
    for (auto pair : this->pointToIterations) {
        if (pair.first == base) {
...
std::string AutonomousDrone::extractCloudPathFromPoint(const Point &base){
    int csv_index = -1;
    for (auto pair : this->pointToIterations) {
        if (pair.first == base) {
            csv_index = pair.second;
    ...
    for (int i=0; i < basePoints.size(); i++){
        ...
        if (currentRoom.exitPoints.size() > 0) {
            if (currentRoom.exitPoints.size()==1) { // TODO : need to check
                ...
                navigateDrone(currentRoom.exitPoints[0], true, false);
            }
            else {
            ...
            if (bfsLevel < 1) { // TODO : change to 2 !!!
        ...
        if (canStart) {
            ...
            while (runDrone) {
                if (!lowBattery) {
                    ...
                    stayInTheAir();
                    getNavigationPoints.join();
                    /*if (stopCondition(currentRoom.exitPoints)) { // TODO : need stop condition
                ...
                rooms.emplace_back(Room());  // TODO : think about it
                currentRoom = rooms.back();
...
std::vector<std::vector<Point>> AutonomousDrone::divide_points(std::vector<Point> basePoints, std::vector<Point> points){
...
void AutonomousDrone::lowBatteryHelper(){
...
void AutonomousDrone::chargerHelper(){
