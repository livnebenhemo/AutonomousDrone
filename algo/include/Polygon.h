...
public:
    ...
    std::vector<Point> getExitPointsByPolygon(bool isDebug = false, bool returnPolygon=false, int angle=10, double ratio=0.5);
    double distanceFromPolygon(const Point& pt);


private:
    std::vector<std::pair<Point, double>> getRawPolygonCorners(int angle=10, double ratio=0.5);
