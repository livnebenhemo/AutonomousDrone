...
namespace ORB_SLAM2 {
    ...
    void LocalMapping::CreateNewMapPoints() {
        ...
        for (size_t i = 0; i < vpNeighKFs.size(); i++) {
            ...
            const double &fx2 = pKF2->fx;
            const double &fy2 = pKF2->fy;
            const double &cx2 = pKF2->cx;
            const double &cy2 = pKF2->cy;
            const double &invfx2 = pKF2->invfx;
            const double &invfy2 = pKF2->invfy;

            // Triangulate each match
            const int nmatches = vMatchedIndices.size();
            for (int ikp = 0; ikp < nmatches; ikp++) {
                auto vMatchedIndice = vMatchedIndices[ikp];
                const int &idx1 = vMatchedIndice.first;
                const int &idx2 = vMatchedIndice.second;
                ...
                double cosParallaxStereo1 = cosParallaxStereo;
                double cosParallaxStereo2 = cosParallaxStereo;
                ...
                double z1 = Rcw1.row(2).dot(x3Dt) + tcw1.at<float>(2);
                if (z1 <= 0)
                    continue;

                double z2 = Rcw2.row(2).dot(x3Dt) + tcw2.at<float>(2);
                if (z2 <= 0)
                    continue;

                //Check reprojection error in first keyframe
                const double &sigmaSquare1 = mpCurrentKeyFrame->mvLevelSigma2[kp1.octave];
                const double x1 = Rcw1.row(0).dot(x3Dt) + tcw1.at<float>(0);
                const double y1 = Rcw1.row(1).dot(x3Dt) + tcw1.at<float>(1);
                const double invz1 = 1.0 / z1;
                double u1 = fx1 * x1 * invz1 + cx1;
                double v1 = fy1 * y1 * invz1 + cy1;
                double errX1 = u1 - kp1.pt.x;
                double errY1 = v1 - kp1.pt.y;
                if ((errX1 * errX1 + errY1 * errY1) > 5.991 * sigmaSquare1)
                    continue;

                //Check reprojection error in second keyframe
                const double sigmaSquare2 = pKF2->mvLevelSigma2[kp2.octave];
                const double x2 = Rcw2.row(0).dot(x3Dt) + tcw2.at<float>(0);
                const double y2 = Rcw2.row(1).dot(x3Dt) + tcw2.at<float>(1);
                const double invz2 = 1.0 / z2;
                double u2 = fx2 * x2 * invz2 + cx2;
                double v2 = fy2 * y2 * invz2 + cy2;
                double errX2 = u2 - kp2.pt.x;
                double errY2 = v2 - kp2.pt.y;
