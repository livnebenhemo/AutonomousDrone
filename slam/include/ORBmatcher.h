...
namespace ORB_SLAM2 {
    ...
    public:
        ...
        int SearchByProjection(KeyFrame *pKF, cv::Mat Scw, const std::vector<std::shared_ptr<MapPoint>> &vpPoints,
                               std::vector<std::shared_ptr<MapPoint>> &vpMatched, int th);

        // Search matches between MapPoints in a KeyFrame and ORB in a Frame.
        // Brute force constrained to ORB that belong to the same vocabulary node (at a certain level)
        // Used in Relocalisation and Loop Detection
        int SearchByBoW(KeyFrame *pKF, Frame &F, std::vector<std::shared_ptr<MapPoint>> &vpMapPointMatches);
        ...
        int SearchBySim3(KeyFrame *pKF1, KeyFrame *pKF2, std::vector<std::shared_ptr<MapPoint>> &vpMatches12, const float &s12,
