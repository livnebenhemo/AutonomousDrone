...
//const std::string currentPointDataFile = "/tmp/currentPointData.csv";
    ...
    while (1) {
        ...
        if (recv(sock, server_reply, 2000, 0) >= 0) {
            ...
            if (command.find(Exit) != std::string::npos) {
                ...
                saveCurrentMap(SLAM->GetTracker()->mCurrentFrame);
                message[0] = Done;
            }
        }
        if (message[0] != 'n') {
