...
// Set Loop edges
        for (
                auto mit = LoopConnections.begin(), mend = LoopConnections.end();
                mit !=
                mend;
                mit++) {
            KeyFrame *pKF = mit->first;
            const long unsigned int nIDi = pKF->mnId;
            auto spConnections = mit->second;
            const g2o::Sim3 Siw = vScw[nIDi];
            const g2o::Sim3 Swi = Siw.inverse();

            for (
                    auto sit = spConnections.begin(), send = spConnections.end();
                    sit !=
                    send;
                    sit++) {
                const long unsigned int nIDj = (*sit)->mnId;
                if ((nIDi != pCurKF->mnId || nIDj != pLoopKF->mnId) && pKF->
                        GetWeight(*sit)
                                                                       < minFeat)
                    continue;
                ...
                e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.
                        vertex(nIDj)
                ));
                e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.
                        vertex(nIDi)
                ));
                e->
                        setMeasurement(Sji);
                ...
                e->
                        ...
                        information() = matLambda;

                optimizer.
                        addEdge(e);

                sInsertedEdges.
                        insert(std::make_pair(std::min(nIDi, nIDj), std::max(nIDi, nIDj))
                );
            }
        }
...
// Loop edges
            auto sLoopEdges = pKF->GetLoopEdges();
...
//cout << "OptimizeEssentialGraph. Covisibility" << endl;
                ...
                if (
                    ...
                    if (!pKFn->
                        ...
                        if (sInsertedEdges.
                                count(std::make_pair(std::min(pKF->mnId, pKFn->mnId), std::max(pKF->mnId, pKFn->mnId))
                        ))
                            continue;
                        ...
                        en->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.
                                vertex(pKFn
                                               ->mnId)));
                        en->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.
                                vertex(nIDi)
                        ));
                        en->
                                setMeasurement(Sni);
    ...
    int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, std::vector<std::shared_ptr<MapPoint>> &vpMatches1,
        ...
        for (size_t i = 0; i < vpEdges12.size(); i++) {
            ...
            if (e12->chi2() > th2 || e21->chi2() > th2) {
                ...
                vpMatches1[idx] = nullptr;// static_cast<MapPoint *>(NULL);
