...
void saveFrame(cv::Mat &img, cv::Mat &pose, int frameCount) {
    ...
    frameData.open(currentWorkingDir + "frameData" +
    ...
    cv::imwrite(
            currentWorkingDir + "frame_" + std::to_string(currentFrameId) + "_" + std::to_string(frameCount) + ".png",
            img);
    frameData.close();
}
...
void saveMap(int mapNumber) {
    ...
    pointData.open(currentWorkingDir + "cloud" + std::to_string(mapNumber) + ".csv");
    int amountOfPoints;
...
int main() {
    ...
    currentWorkingDir = std::string(currentDirPath) + "/../simulatorDataSets/" + currentTime + "/";
