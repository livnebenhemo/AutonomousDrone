...
namespace ORB_SLAM2 {
    ...
    int
    ORBmatcher::SearchByProjection(Frame &F, const vector <std::shared_ptr<MapPoint>> &vpMapPoints, const float th) {
        int nmatches = 0;

        const bool bFactor = th != 1.0;

        for (size_t iMP = 0; iMP < vpMapPoints.size(); iMP++) {
            auto pMP = vpMapPoints[iMP];
    ...
    int ORBmatcher::SearchByBoW(KeyFrame *pKF, Frame &F, vector <std::shared_ptr<MapPoint>> &vpMapPointMatches) {
        ...
        vpMapPointMatches = vector<std::shared_ptr<MapPoint>>(F.N, nullptr);
        ...
        if (mbCheckOrientation) {
            ...
            for (int i = 0; i < HISTO_LENGTH; i++) {
                ...
                for (size_t j = 0, jend = rotHist[i].size(); j < jend; j++) {
                    vpMapPointMatches[rotHist[i][j]] = nullptr;//static_cast<MapPoint *>(NULL);
                    nmatches--;
                }
            }
        }
    ...
    int ORBmatcher::SearchByProjection(KeyFrame *pKF, cv::Mat Scw, const vector <std::shared_ptr<MapPoint>> &vpPoints,
                                       vector <std::shared_ptr<MapPoint>> &vpMatched, int th) {
    ...
    int ORBmatcher::SearchByBoW(KeyFrame *pKF1, KeyFrame *pKF2, vector <std::shared_ptr<MapPoint>> &vpMatches12) {
        ...
        vpMatches12 = vector<std::shared_ptr<MapPoint>>(vpMapPoints1.size(), nullptr);
        ...
        if (mbCheckOrientation) {
            ...
            for (int i = 0; i < HISTO_LENGTH; i++) {
                if (i == ind1 || i == ind2 || i == ind3)
                    continue;
                for (size_t j = 0, jend = rotHist[i].size(); j < jend; j++) {
                    vpMatches12[rotHist[i][j]] = nullptr;// static_cast<MapPoint *>(NULL);
                    nmatches--;
                }
            }
        }
    ...
    int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono) {
        ...
        if (mbCheckOrientation) {
            ...
            for (int i = 0; i < HISTO_LENGTH; i++) {
                if (i != ind1 && i != ind2 && i != ind3) {
                    for (size_t j = 0, jend = rotHist[i].size(); j < jend; j++) {
                        CurrentFrame.mvpMapPoints[rotHist[i][j]] = nullptr;// static_cast<MapPoint *>(NULL);
                        nmatches--;
                    }
                }
            }
    ...
    int ORBmatcher::SearchByProjection(Frame &CurrentFrame, KeyFrame *pKF,
                                       const set <std::shared_ptr<MapPoint>> &sAlreadyFound,
